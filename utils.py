...
def get_coords(cocycle, threshold, num_sampled, dists, coeff):
    ...
    f, istop, itn, normr, normar = lsmr(Aw, Bw)[0:5]
    
    return f, verts, normr, normar
...
def get_spikes(rat_name, mod_name, day_name = '', sess_name = '', bType = None, bSmooth = True, 
               bBinned = True, bSpeed = True, bStart = False, smoothing_width = -1, folder = '', SIM=False, case = 0, seed_clus = 1., ts=None, speed_thresh = 2.5, vel = 'fast', binning = 10):
    ...
    if SIM==True:
        f = np.load(case, allow_pickle = True)
        spikes_all = f['arr_0'][()]
...
#         spikes_all = {}
#         
#         sspikes = np.loadtxt('resample_'+str(rat_name)+''+str(mod_name)+'_'+str(day_name)+'_'+str(ts)+'.txt')
#         #sspikes = np.loadtxt('poisson_grid_spikes_%s_1_m.txt'%(int(seed_clus)))
#         for i in range(sspikes.shape[1]):
#             spikes_all[i] = []
#         t0 = 7457
#         for i, train in enumerate(sspikes.T):
#             for t, nbin in enumerate(train):
#                 for n in range(int(nbin)):
#                     spikes_all[i].append(t0 + t/100 + 0.005)
#             spikes_all[i] = np.array(spikes_all[i])
# =============================================================================
    for i,train in enumerate(spikes_all.values()):
        noise = np.random.normal(0., ts, size=train.shape)
        spikes_all[i] = train + noise     
    ...
    if bType:    
        ...
        if len(day_name)>0:
            is_conj = f['is_conj_' + str(rat_name) + mod_name + '_' + day_name]
        else:
            is_conj = f['is_conj_' + str(rat_name) + mod_name + day_name]
        f.close()
        if bType == 'conj':
    ...
    if sess_name.find('_')>-1: 
        ...
        ind = f[str(rat_name) + mod_name + '_ind']
        f.close()
    ...
    if str(sess_name[:3]) in ('SWS', 'REM'):
        times = times_all['rat_' + str(rat_name).lower() + '_sleep' + day_name]
    elif str(sess_name[:3]) == 'WW2':
        times = times_all['rat_' + str(rat_name).lower() + '_' + sess_name + day_name]        
    else:
        times = times_all['rat_' + str(rat_name).lower() + '_' + sess_name[:2] + day_name]        
    ...
    if not bBinned:
        ...
        if (rat_name =='S') & (sess_name == 'WW'):
            times2 = times_all['rat_' + str(rat_name).lower() + '_' + sess_name + '2' + day_name]        
        ...
        if sess_name[:2] in ('OF', 'WW'):
            if bSpeed:
                ...
                if vel == 'fast':
                    ssp  = np.where(speed>speed_thresh)[0]
                elif vel == 'slow':
                    ssp  = np.where(speed<speed_thresh)[0]
            ...
            start = np.array(times_all['rat_' + str(rat_name).lower() + '_' + sess_name[:3] + day_name])[:,0]
            start = start[start<max_time0]

            end = np.array(times_all['rat_' + str(rat_name).lower() + '_' + sess_name[:3] + day_name])[:,1]
        ...
        if sess_name[:2] in ('OF', 'WW'):
            ...
            start = np.array(times_all['rat_' + str(rat_name).lower() + '_' + sess_name[:3] + day_name])[:,0]*res
            start = start[start<max_time]

            end = np.array(times_all['rat_' + str(rat_name).lower() + '_' + sess_name[:3] + day_name])[:,1]*res
        ...
        if bSpeed: 
            xx, yy, aa, tt, speed = load_pos(rat_name, sess_name[:2], day_name, bSpeed = True, folder = folder, binning = binning)
            
            #fast
            if vel == 'fast':
                spikes_bin = spikes_bin[speed>speed_thresh,:]            
                xx = xx[speed>speed_thresh]
                yy = yy[speed>speed_thresh]
                aa = aa[speed>speed_thresh]
                tt = tt[speed>speed_thresh]
            elif vel == 'slow':
                spikes_bin = spikes_bin[speed<speed_thresh,:]            
                xx = xx[speed<speed_thresh]
                yy = yy[speed<speed_thresh]
...
def load_pos(rat_name, sess_name, day_name = '', bSpeed = False, folder = '',  binning = 10):    
    if str(rat_name) == 'R':
        f = np.load(folder + 'rat_r_' + day_name + '_grid_modules_1_2_3.npz', allow_pickle = True)
    elif str(rat_name) == 'Q':
        f = np.load(folder + 'rat_q_grid_modules_1_2.npz', allow_pickle = True)
    ...
    if (str(rat_name) == 'R') & (day_name == 'day1'):
        if sess_name =='OF':
            valid_times = np.concatenate((np.arange(0, (14778-7457)*res/dt),
                                          np.arange((14890-7457)*res/dt, (16045-7457)*res/dt))).astype(int)
        elif sess_name == 'WW':
    ...
    if bSpeed:
        ...
        speed = np.concatenate(([speed[0]],speed))
        if (str(rat_name) =='S') & (sess_name == 'WW'):
            xx1, yy1, aa1, tt1, speed1 = load_pos(rat_name, 'WW2', day_name, True, folder = folder, binning=binning)
    ...
    if (str(rat_name) =='S') & (sess_name == 'WW'):
        xx1, yy1, aa1, tt1 = load_pos(rat_name, 'WW2', folder = folder, binning = binning)
        xx = np.concatenate((xx,xx1))
        yy = np.concatenate((yy,yy1))
        aa = np.concatenate((aa,aa1))
        tt = np.concatenate((tt,tt1))
...
def plot_barcode(persistence, file_name = ''):
    ...
    for dit, dim in enumerate(dims):
        ...
        axes.set_xlim([0, 20])
        plt.ylabel(labels[dit], fontsize = 60, rotation = 0, labelpad = 35)
...
def toroidal_alignment(rat_name, mod_name, sess_name_0, sess_name_1, day_name, bPlot = False, folder = ''):
    ...
    return np.cos(pm00),np.cos(pm01)
...
def fit_para(rat_name, mod_name, sess_name, day_name, folder = ''):
    ...
    #p1b_1 and p2b_1 are period phase and rotation of the cosine wave for the two cocycles
...
#     yy = yy[speed>2.5]
#     
#     times_box_1 = f1['times_box']
#     times_box_2 = f2['times_box']
#     __, t_box_1, t_box_2  = np.intersect1d(times_box_1, times_box_2, assume_unique=True, return_indices = True)
# 
#     
#     xx = xx[times_box_1[t_box_1]]
